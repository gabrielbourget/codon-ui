...
This records the Amino UI-side plan for receiving Wavemap's reusable `DateTimePicker` as a prerequisite for the richer
`add table` graph. The immediate goal is to satisfy Table date/date-time filter argument support without weakening the
public `add table` scope or pulling Wavemap app adapters into Amino UI.
...
| `apps/wavemap-front-end/src/components/DateTimePicker/DateTimePickerStyles.module.css` | Receive after CSS variable rewrites to the default Amino theme contract.                                                               |
...
Do not add Wavemap path alias support to Amino UI.
...
does not directly import it. Do not add it to `@amino-ui/react` because of this receipt alone.

## Theme And CSS Rewrite Draft

The receipt should rewrite legacy Wavemap CSS aliases directly to the existing Amino default contract where possible:
...
| ------------------------------------- | -------------------------------------------------------------------------------------------- |
| `DateTimePicker/i18n.ts`              | Wavemap i18n adapter using app i18n utilities and packages. Amino core should expose labels. |
...
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| Source proof       | `pnpm -F @amino-ui/react test` after adding `verify-date-time-picker-proof.mjs`.                                                                      |
| Package checks     | `pnpm -F @amino-ui/react format:check`, `pnpm -F @amino-ui/react lint`, `pnpm -F @amino-ui/react stylelint`, and `pnpm -F @amino-ui/react typecheck`. |
| Registry contracts | `pnpm -F @amino-ui/react check:contracts` after manifest/snapshot activation.                                                                         |
| Build              | `pnpm build:react` once package source and public exports are active.                                                                                 |
| CLI registry tests | `pnpm -F aminoui-cli test` after local registry snapshots change.                                                                                     |
...
- Adding Wavemap import aliases to Amino UI.
